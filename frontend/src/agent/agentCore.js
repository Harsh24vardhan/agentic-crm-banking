...
/**
 * Runs a simulated ReAct loop that executes the real JS tools.
 * Returns an object containing the detailed execution trace steps and the final results.
 * @param {string} query 
 * @returns {Object} { steps: Array, leads: Array, productType: string }
 */
export function runAgent(query) {
  ...
  rawCustomers.forEach(customer => {
    const txResult = get_customer_transactions(customer.id);
    if (txResult.success) {
  ...
  const scoredLeads = [];
  rawCustomers.forEach(customer => {
    const scoreResult = calculate_conversion_probability(customer.id, productType);
  ...
  const finalLeads = scoredLeads.map(lead => {
    const msgResult = generate_personalized_message(lead.customerId, productType, "WhatsApp");
    return {
      ...lead,
